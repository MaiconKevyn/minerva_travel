...
test('create guide wizard asks for destination first and uses five steps', () => {
  ...
  assert.match(page, /case 2: return <Step4Attractions \/>;/);
  assert.match(page, /case 3: return <EnhancedStep5FamilyDetails \/>;/);
  assert.match(page, /case 4: return <Step2CoverPhoto \/>;/);
  assert.match(page, /case 5: return <Step5Review \/>;/);
  assert.match(page, /\[1, 2, 3, 4, 5\]/);
  assert.match(page, /Passo \{currentStep\} de 5/);
  assert.match(context, /Math\.min\(currentStep \+ 1, 5\)/);
});
...
test('cover photo step comes after route confirmation and stays before review', () => {
  ...
  const photoCaseIndex = page.indexOf('case 4: return <Step2CoverPhoto />;');
  const reviewCaseIndex = page.indexOf('case 5: return <Step5Review />;');
